...
4.Create browser router takes the list of paths of object in the array.
...
6.We can create our own errror page, to create our own error we have another key in the path object, errorElement in the CreateBrowserRouter
7.React Router DOM give us a hook that useRouteError. It is used to give some more information about why the error is occured. Whenever an error occured react will give a message as an object using useRouteError hook.
...
2.If we want to out header and footer on its place and only want to change the body or some other component then we have to use children key in createBrowserRouter and Outlet component.
3.Whenever we have to navigate to some urls by clicking on a button, never use achor tag. beacuse whole page get refreshed using anchor tag.
4.Use Link component instead of anchor tag which is given by react-router-dom. If you use Link component then the whole page will not refresh instead the component will refresh that why we called React applications are "Single Page Application" SPA
...
1.Server Side Routing  --> Suppose, we have a home.html, about.html, contact.html and when i click on any of the links it will call the request of that url and send the response back that is called server side routing.
